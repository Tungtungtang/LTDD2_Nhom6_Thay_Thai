...
Button btnAddDrink;
                ...
                if(MainActivity.manggiohang.size()> 0)
                {
                    ...
                    for(int i= 0;i < MainActivity.manggiohang.size(); i++)
                    {
                        if(MainActivity.manggiohang.get(i).getId_hoadon()== iddrink)
                        {
                            ...
                            MainActivity.manggiohang.get(i).setGia(GiaDrink* MainActivity.manggiohang.get(i).getSoLuong());
                            exits = true;
                        }
                    }
                    if (exits  == false ){
                        int soluong =Integer.parseInt( spinner.getSelectedItem().toString());
                        int giamoi =soluong * GiaDrink;
                        MainActivity.manggiohang.add(new HoaDon(iddrink,TenChiTietDrink,giamoi,soluong));
                    }
                }
                else
                {
   ...
   private void getSanPhamDrink()
   {
       ...
       priceprocut.setText(decimalFormat.format(GiaDrink)+ "Đ");
